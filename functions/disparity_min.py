# ============================================================================
# functions/disparity_min.py
# ============================================================================
from submodlib.functions.disparityMin import DisparityMinFunction

class DisparityMinSelector:
    """Frame selection using Disparity Min submodular function with memoization.
    
    Disparity Min minimizes the maximum similarity between selected frames and
    the rest of the frames. This encourages selecting diverse frames that are
    maximally dissimilar to unselected frames.
    
    Use Case: Maximum diversity selection, outlier detection
    """
    ...
    def __init__(self, budget=64, metric="cosine", delta=None, use_delta=None):
        ...
        self.dm_function = None  # Store function instance for memoization control
    ...
    def select(self, feature_matrix):
        """
        Select frames using Disparity Min with optional delta expansion.
        ...
        """
        try:
            self.dm_function = DisparityMinFunction(
                n=len(feature_matrix),
                mode="dense",
                data=feature_matrix,
                metric=self.metric
            )
            selected = self.dm_function.maximize(budget=self.budget, optimizer="NaiveGreedy")
            selected_indices = sorted([t[0] for t in selected])
            
            print(f"[INFO] DisparityMin selected frames: {selected_indices}")
            ...
            print(f"[ERROR] DisparityMin selection failed: {e}")
    ...
    def clear_memoization(self):
        ...
        if self.dm_function is not None:
            try:
                self.dm_function.clearMemoization()
                print("[INFO] DisparityMin memoization cleared.")
            except Exception as e:
                print(f"[WARN] Failed to clear memoization: {e}")
        else:
            print("[WARN] No DisparityMin function to clear.")
